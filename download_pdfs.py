...
folder_name = "Y:/AI Internship Task Wasserstoff/dataset"
...
# Dictionary containing PDF URLs
pdfs = {
...
def download_pdfs(pdfs, folder_name):
    for pdf_name, pdf_url in pdfs.items():
        try:
            response = requests.get(pdf_url)
            response.raise_for_status()  # Check for HTTP errors
            
            pdf_file_path = os.path.join(folder_name, f"{pdf_name}.pdf")
            print(f"Attempting to write to: {pdf_file_path}")  # Debugging statement
...
# Call the function
download_pdfs(pdfs, folder_name)
